...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # Interactive piece 3: inputId = "select_occupation"
      checkboxGroupInput(inputId = "select_occupation", label = "Include Occupations:", 
...
server <- function(input, output) {
  ...
  create_subset <- reactive({income %>%
                              # Only include capital loss > 0
                              filter(capital_loss > 0  &
                                       # income category is the input selected above ("subset_income"),
                                       income == input$subset_income &
                                       # occupation is in the input selected above ("subset_occupation")
                                      occupation %in% input$select_occupation)})
